...
class TelegramBot:
    ...
    def is_running(self):
        ...
        return self._running and hasattr(self.application,
                                         'updater') and self.application.updater and self.application.updater.running
    ...
    def setup_handlers(self):
        ...
        try:
            ...
            conv_handler = ConversationHandler(
                entry_points=[
                    CallbackQueryHandler(self.start_conversation,
                                         pattern="^(settings_add_pair|settings_remove_pair|settings_tp_multipliers|threshold_.*)$"),
                    CallbackQueryHandler(self.handle_callback, pattern="^(subscribers_add|subscribers_remove)$")
                ],
                states={
                    WAITING_ADD_SUBSCRIBER: [
                        MessageHandler(filters.TEXT & ~filters.COMMAND, self.add_subscriber_callback)],
                    WAITING_REMOVE_SUBSCRIBER: [
                        MessageHandler(filters.TEXT & ~filters.COMMAND, self.remove_subscriber_callback)],
                    WAITING_THRESHOLD_CHANGE: [
                        MessageHandler(filters.TEXT & ~filters.COMMAND, self.change_threshold_callback)],
                    ...
                    WAITING_TP_MULTIPLIERS: [
                        MessageHandler(filters.TEXT & ~filters.COMMAND, self.tp_multipliers_callback)],
                },
                fallbacks=[
                    CommandHandler("cancel", self.cancel),
                    CallbackQueryHandler(self.cancel_conversation,
                                         pattern="^(back_to_main|manage_subscribers|settings|settings_pairs)$")
                ],
                per_chat=True,
                per_user=True,
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        elif data.startswith("settings_") and not any(
                data == ep for ep in ["settings_add_pair", "settings_remove_pair", "settings_tp_multipliers"]):
...
"""
            ...
            keyboard = [
                ...
                [
                    InlineKeyboardButton(f"🐋 WHALE TRACKING: {'✅' if whale_tracking else '❌'}",
                                         callback_data="filter_whale_tracking"),
                    InlineKeyboardButton(f"🕵️ SPOOFING: {'✅' if spoofing_detection else '❌'}",
                                         callback_data="filter_spoofing_detection")
                ],
                [
                    InlineKeyboardButton(f"📏 SPREAD FILTER: {'✅' if spread_filter else '❌'}",
                                         callback_data="filter_spread_filter"),
                    InlineKeyboardButton(f"📈 TREND MATCH: {'✅' if trend_match else '❌'}",
                                         callback_data="filter_trend_match")
                ],
                [
...
"""
            ...
            keyboard = [
                [
                    ...
                    InlineKeyboardButton(f"🎭 SPOOFING {'✅' if spoofing_detection else '❌'}",
...
"""
                    ...
                    volume_formatted = f"{data['volume_24h']:,.0f}" if data[
                                                                           'volume_24h'] > 1000 else f"{data['volume_24h']:.2f}"
...
"""
    ...
    async def force_scan(self, query):
        ...
        try:
            ...
            # Get monitored pairs
            scanner_status = db.get_scanner_status()
            monitored_pairs = json.loads(
                scanner_status.get('monitored_pairs', '["BTCUSDT", "ETHUSDT", "ADAUSDT", "BNBUSDT", "XRPUSDT"]'))
    ...
    async def remove_pair_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if len(current_pairs) <= 1:
                await update.message.reply_text(
                    "❌ **Cannot remove the last pair!** At least one pair must be monitored.")
                return ConversationHandler.END
